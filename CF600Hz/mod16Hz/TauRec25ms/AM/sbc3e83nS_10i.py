...
def mksbc3e83nS_10i(anfSpkFile, temp_degC=37):
    ...
    nSbcs = 4
    nAnfsPerSbc = 3
    nInhInputsPerSbc = 10 # Xie & Manis (2013) model; add 1-ms delay
    #nGbcs = 4
    #nAnfsPerGbc=30
    ...
    delay4Inh = 1.3*ms
    anfSpkTimes_delay4Inh = anfSpkTimes + delay4Inh
    anfInhSpkGeneratorGrp = SpikeGeneratorGroup(nANF, anfIndices, anfSpkTimes_delay4Inh)
    ...
    Es_e = 0.*mV
    tausE = 0.2*ms
    Es_i = -75.*mV # Kuenzel et al. (2015)
    tausI = 23.9*ms # Kuenzel et al. (2015)
...
#    #sSynEState = StateMonitor(sbcSynE, ['g_e','gs_e_post'], record=True)
    ...
    sbc3_10SynI = Synapses(anfInhSpkGeneratorGrp, sbc3Grp, 
                     model='''dg_i/dt = -g_i/tausI : siemens (clock-driven)
                              gs_i_post = g_i : siemens (summed)''',
                     on_pre='g_i += w_isbc*siemens')
    sbc3_10SynI.connect(i=np.arange(0, nInhInputsPerSbc), j=0)
    sbc3_10SynI.connect(i=np.arange(nInhInputsPerSbc, 2*nInhInputsPerSbc), j=1)
    sbc3_10SynI.connect(i=np.arange(2*nInhInputsPerSbc, 3*nInhInputsPerSbc), j=2)
    sbc3_10SynI.connect(i=np.arange(3*nInhInputsPerSbc, 4*nInhInputsPerSbc), j=3)
...
#    file1.close()
    ...
    sbc3SpkFile = 'sb383_10iSpT' + anfSpkFile[10:len(anfSpkFile)]
