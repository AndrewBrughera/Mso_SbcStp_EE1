...
def mksbc3SyDzrest(anfSpkFile, temp_degC=37):
...
#    """
    ...
    # KLT channel (low threshold K+) with inactivation z frozen (at z_rest)
    eqs_klt = """
    iklt = gkltbar*w**4*z*(EK-v) : amp
    dw/dt=q10*(winf-w)/wtau : 1
    dz/dt=0.0*Hz : 1
    winf = (1. / (1 + exp(-(vu + 48.) / 6.)))**0.25 : 1
    zinf = zss + ((1.-zss) / (1 + exp((vurest + 71.) / 10.))) : 1
    wtau = ((100. / (6.*exp((vu+60.) / 6.) + 16.*exp(-(vu+60.) / 45.))) + 1.5)*ms : second
    ztau = ((1000. / (exp((vu+60.) / 20.) + exp(-(vu+60.) / 8.))) + 50)*ms : second
    """
...
#    #sbcGrp.gs_e = 0.0*siemens
    ...
    sbc3Grp.v = vrest
    vu = EL/mV # unitless v
    vurest = vrest/mV # unitless vrest
    ...
    sbc3Grp.z = zss + ((1.-zss) / (1 + exp((vurest + 71.) / 10.)))
...
#    file1.close()
    ...
    sbc3SpkFile = 'sb3SyDzrSpTMms' + anfSpkFile[10:len(anfSpkFile)]
