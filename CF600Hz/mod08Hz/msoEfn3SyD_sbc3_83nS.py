...
def mkmsoEnSyD(inputSpkFileTuple, temp_degC=37):
...
#    """
    ...
    Im = gNabar*m**4*((0.993*h) + 0.007)*(ENa-v) + gKltbar*w**4*z*(EK-v) + gHbar*r*(EH-v) + gL*(EL-v) : amp/meter**2
    #
    Is_eCo0 = gs_eCo0 * (Es_e - v) : amp (point current)
    dgs_eCo0/dt = -gs_eCo0/taus_e : siemens
    dw_eCo0/dt = (w0_eCo - w_eCo0)/tauSyDe : 1
    #
    Is_eCo1 = gs_eCo1 * (Es_e - v) : amp (point current)
    dgs_eCo1/dt = -gs_eCo1/taus_e : siemens
    dw_eCo1/dt = (w0_eCo - w_eCo1)/tauSyDe : 1
    #
    Is_eCo2 = gs_eCo2 * (Es_e - v) : amp (point current)
    dgs_eCo2/dt = -gs_eCo2/taus_e : siemens
    dw_eCo2/dt = (w0_eCo - w_eCo2)/tauSyDe : 1
    #
    Is_eCo3 = gs_eCo3 * (Es_e - v) : amp (point current)
    dgs_eCo3/dt = -gs_eCo3/taus_e : siemens
    dw_eCo3/dt = (w0_eCo - w_eCo3)/tauSyDe : 1
    #
    Is_eIp0 = gs_eIp0 * (Es_e - v) : amp (point current)
    dgs_eIp0/dt = -gs_eIp0/taus_e : siemens
    dw_eIp0/dt = (w0_eIp - w_eIp0)/tauSyDe : 1
    #
    Is_eIp1 = gs_eIp1 * (Es_e - v) : amp (point current)
    dgs_eIp1/dt = -gs_eIp1/taus_e : siemens
    dw_eIp1/dt = (w0_eIp - w_eIp1)/tauSyDe : 1
    #
    Is_eIp2 = gs_eIp2 * (Es_e - v) : amp (point current)
    dgs_eIp2/dt = -gs_eIp2/taus_e : siemens
    dw_eIp2/dt = (w0_eIp - w_eIp2)/tauSyDe : 1
    #
    Is_eIp3 = gs_eIp3 * (Es_e - v) : amp (point current)
    dgs_eIp3/dt = -gs_eIp3/taus_e : siemens
    dw_eIp3/dt = (w0_eIp - w_eIp3)/tauSyDe : 1
    #
    """
    ...
    # Synapses
    '''Synapses individual for independent synaptic depression
    '''
    # Contrlateral Excitatory
    Syn_eCo0 = Synapses(sbCoSpkGenGrp, nrn, on_pre='''gs_eCo0 += w_eCo0*siemens
                                                     w_eCo0 = w_eCo0*(1 - u)''')
    Syn_eCo0.connect(i=0, j=morpho.cDend[150.*(0.4 + 0.025)*um])
    Syn_eCo0.w_eCo0 = w0_eCo
    
    Syn_eCo1 = Synapses(sbCoSpkGenGrp, nrn, on_pre='''gs_eCo1 += w_eCo1*siemens
                                                     w_eCo1 = w_eCo1*(1 - u)''')
    Syn_eCo1.connect(i=1, j=morpho.cDend[150.*(0.45 + 0.025)*um])
    Syn_eCo1.w_eCo1 = w0_eCo
    
    Syn_eCo2 = Synapses(sbCoSpkGenGrp, nrn, on_pre='''gs_eCo2 += w_eCo2*siemens
                                                     w_eCo2 = w_eCo2*(1 - u)''')
    Syn_eCo2.connect(i=2, j=morpho.cDend[150.*(0.5 + 0.025)*um])
    Syn_eCo2.w_eCo2 = w0_eCo
    
    Syn_eCo3 = Synapses(sbCoSpkGenGrp, nrn, on_pre='''gs_eCo3 += w_eCo3*siemens
                                                     w_eCo3 = w_eCo3*(1 - u)''')
    Syn_eCo3.connect(i=3, j=morpho.cDend[150.*(0.55 + 0.025)*um])
    Syn_eCo3.w_eCo3 = w0_eCo
    
    # Ipsilateral Excitatory
    Syn_eIp0 = Synapses(sbIpSpkGenGrp, nrn, on_pre='''gs_eIp0 += w_eIp0*siemens
                                                     w_eIp0 = w_eIp0*(1 - u)''')
    Syn_eIp0.connect(i=0, j=morpho.iDend[150.*(0.4 + 0.025)*um])
    Syn_eIp0.w_eIp0 = w0_eIp
    
    Syn_eIp1 = Synapses(sbIpSpkGenGrp, nrn, on_pre='''gs_eIp1 += w_eIp1*siemens
                                                     w_eIp1 = w_eIp1*(1 - u)''')
    Syn_eIp1.connect(i=1, j=morpho.iDend[150.*(0.45 + 0.025)*um])
    Syn_eIp1.w_eIp1 = w0_eIp
    
    Syn_eIp2 = Synapses(sbIpSpkGenGrp, nrn, on_pre='''gs_eIp2 += w_eIp2*siemens
                                                     w_eIp2 = w_eIp2*(1 - u)''')
    Syn_eIp2.connect(i=2, j=morpho.iDend[150.*(0.5 + 0.025)*um])
    Syn_eIp2.w_eIp2 = w0_eIp
    
    Syn_eIp3 = Synapses(sbIpSpkGenGrp, nrn, on_pre='''gs_eIp3 += w_eIp3*siemens
                                                     w_eIp3 = w_eIp3*(1 - u)''')
    Syn_eIp3.connect(i=3, j=morpho.iDend[150.*(0.55 + 0.025)*um])
    Syn_eIp3.w_eIp3 = w0_eIp
...
#    Syn_iIp.w_iIp = w0_iIp
    ...
    PhsStrCo = sbCoSpkFile[31:35]
    PhsStrIp = sbIpSpkFile[31:35]
    if (PhsStrCo[0] == 'N'):
    ...
    while (len(IPDstr) < 3):
        IPDstr = '0' + IPDstr
    msoSpkFile = 'msonSyDSpT83s3CF' + sbCoSpkFile[14:19] + 'StIPD' + IPDstr + 'C' + sbCoSpkFile[26:len(sbCoSpkFile)]
