% ANSR100kHzAMStaticIPD1Species3.m
% Applying the Zilany et al. 2014 model with AM and Static IPD
% (a control condition for amplitude modulated binaural beats stimuli (AMBB)
% as in Dietz et al. 2013 PNAS, 2014 JNrPhys
...
spikeTimes_s = cell([length(CFs_Hz) length(ModFreqs_Hz) length(StartPhases_deg) length(fiberTypeStrings) nANFsPerFiberType]);
% Synchrony Index (help synccalc)
syncIdx   = -1 * ones([length(CFs_Hz) length(ModFreqs_Hz) length(StartPhases_deg) length(fiberTypeStrings) nANFsPerFiberType]);
% Rayleigh Criterion for significance: 2*number_of_spikes*(syncIdx^2) > 13.8
two_n_Rsq  = -1 * ones([length(CFs_Hz) length(ModFreqs_Hz) length(StartPhases_deg) length(fiberTypeStrings) nANFsPerFiberType]);
    ...
    for idxCF = 1:length(CFs_Hz)
        ...
        for idxModF = 1:length(ModFreqs_Hz)
            ...
            for idxStartPhase = 1:length(StartPhases_deg)
                ...
                StPhsDegString = num2str(StartPhase_deg);
                leadingZeros = '';
                if (StartPhase_deg == 0)
                    StPhsDegString = '000';
                elseif (StartPhase_deg < 0)
                    StPhsDegString(1) = 'N';
                    if (StartPhase_deg > -10)
                        leadingZeros = '00';
                    elseif (StartPhase_deg > -100)
                        leadingZeros = '0';
                    end
                    StPhsDegString =[StPhsDegString(1) leadingZeros StPhsDegString(2:end)];
                elseif (StartPhase_deg > 0)
                    if (StartPhase_deg < 10)
                        leadingZeros = '00';
                    elseif (StartPhase_deg < 100)
                    ...
                    StPhsDegString =[leadingZeros StPhsDegString];
                end
                
                % Apply the amplitude modulation at the ModFreq_Hz
                AM = (1 - cos(2*pi*ModFreqHz*t_s))/2;
                ...
                for fiberType = 1:nFiberTypes
                    ...
                    filename = ['ANSpTs' 'CF' num2str(CF_Hz) 'Hz' 'Ca' num2str(F0_Hz) 'Hz' num2str(ear) 'StPhs' StPhsDegString 'AM' num2str(ModFreqHz) 'Hz' num2str(stimI_dBSPLrms) 'dBspl' fiberTypeStringsShort{fiberType} 'Spec' num2str(species)];
